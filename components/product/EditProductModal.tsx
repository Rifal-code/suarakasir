...
export default function EditProductModal({ isOpen, productId, initialData, onClose, onSuccess }: EditProductModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If imagePreview is null, it means the user deleted the image.
      // If there's an imageFile, they uploaded a new one.
      // Otherwise, keep the original (or set to empty if deleted).
      let imageUrl = imagePreview ? initialData.imageUrl || "" : "";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-card w-full max-w-lg rounded-3xl shadow-2xl border border-border-default overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-border-soft flex justify-between items-center bg-white z-10 shrink-0">
          <div>
            <h2 className="text-xl font-bold text-text-primary tracking-tight">Edit Produk</h2>
            <p className="text-xs text-text-secondary mt-0.5">Perbarui informasi produk di katalog</p>
          </div>
          <button 
            onClick={onClose}
